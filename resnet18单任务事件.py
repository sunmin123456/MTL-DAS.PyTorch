...
# 训练/验证过程
def train_event(model, epoch_num, start_epoch, optimizer, criterion, data_loader, save_dir,
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        if epoch % 5 == 0:
            ...
            print("{}\nepoch:{}  正确率: event:{}".format("*" * 50, epoch, acc1))
            ...
            for taskindex, taskname in enumerate(['任务2：event']):
                print('任务2：{}'.format(taskname))
...
def main(ch=16, k_size=7, stride=3, res_num=8, task='event', is_test=False, pth_file='',random_state=1,fold_index=1):
